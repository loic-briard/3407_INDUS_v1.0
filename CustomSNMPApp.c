...
extern unsigned char 
	UC_LCD_LIGNE[16];		// variables liées au lcd 
...
*/
	if((!UC_LCD_LIGNE[14] && !UC_LCD_LIGNE[15]) && (trapIndex >= TRAP_TABLE_SIZE))
	{
		asciiStrNotify = FALSE;
	}
	else if((UC_LCD_LIGNE[14] && UC_LCD_LIGNE[15]) && (trapIndex < TRAP_TABLE_SIZE))
		asciiStrNotify = TRUE;
...
BOOL SNMPSetVar(SNMP_ID var, SNMP_INDEX index, BYTE ref, SNMP_VAL val)
{
    ...
    {
    case LED_D5:
        UC_LCD_LIGNE[14] = (val.byte+0x30);
        putrsUART(UC_LCD_LIGNE[14]);
        return TRUE;

    case LED_D6:
        UC_LCD_LIGNE[15] = (val.byte+0x30);
        putrsUART(UC_LCD_LIGNE[15]);
        return TRUE;
#if !defined(SNMP_TRAP_DISABLED)
...
BOOL SNMPGetVar(SNMP_ID var, SNMP_INDEX index, BYTE* ref, SNMP_VAL* val)
{
    ...
    case LED_D5:
        val->byte = (UC_LCD_LIGNE[14]-0x30);
        return TRUE;

    case LED_D6:
        val->byte = (UC_LCD_LIGNE[15]-0x30);
        return TRUE;
